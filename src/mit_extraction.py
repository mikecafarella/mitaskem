...
async def _extract_text_vars(text, var_prompt, gpt_key):
    ...
    res = await fork_join_requests(gpt_key, task_prompts, model=model_name)
...
async def afind_vars_from_text(text: str, gpt_key: str):
    ...
    start = time.time()
    res = await _extract_text_vars(text, var_prompt, gpt_key)
